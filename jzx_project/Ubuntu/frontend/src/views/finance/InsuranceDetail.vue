  ...
  <div v-loading="loading">
    ...
    <el-card shadow="hover" style="margin-bottom: 20px;">
      ...
      <el-descriptions :column="3" border>
        <el-descriptions-item label="公司编码">{{ detail.code || '-' }}</el-descriptions-item>
...
async function loadDetail() {
  loading.value = true
  try {
    const res = await request.get(`/dict/insurances/${route.params.id}`)
    detail.value = res.data || {}
  } catch (e) {
    console.error('加载保险公司详情失败:', e)
  } finally {
    loading.value = false
  }
}
...
async function loadWorkOrders() {
  ...
  try {
    const res = await request.get(`/dict/insurances/${route.params.id}/work-orders`, {
...
function goBack() {
  router.push('/finance/insurances')
}
