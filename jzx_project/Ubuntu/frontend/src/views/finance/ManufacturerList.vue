<template>
  <div class="page-container">
    <el-card shadow="hover">
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <span>索赔厂家管理</span>
          <el-button type="primary" @click="handleAdd">新增厂家</el-button>
        </div>
      </template>
      <el-table :data="list" border stripe v-loading="loading">
        <el-table-column prop="id" label="ID" width="60" />
        <el-table-column prop="code" label="厂家编码" width="120">
          <template #default="{ row }">
            <el-link type="primary" @click="$router.push(`/finance/manufacturers/${row.id}`)">{{ row.code || '-' }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" align="center">
          <template #default="{ row }">
            ...
            <el-popconfirm title="确定删除此厂家？" @confirm="handleDelete(row.id)">
              <template #reference>
    ...
    <el-dialog v-model="showDialog" :title="isEdit ? '编辑厂家' : '新增厂家'" width="500px" @close="resetForm">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="厂家编码" prop="code">
          <el-input v-model="form.code" placeholder="请输入厂家编码" :disabled="isEdit" />
        </el-form-item>
...
const rules = { code: [{ required: true, message: '请输入厂家编码', trigger: 'blur' }], name: [{ required: true, message: '请输入厂家名称', trigger: 'blur' }] }
...
<style scoped>
/* Apple Design - Manufacturer List Page Styles */
